    ...
    <div className="sidebar p-3 d-flex flex-column">
      ...
      <div className="mt-4">
        ...
        <div className={`nav-item ${active === "profile" && "active"}`}
          ...
          <img className = "me-2" src='/user.png' style={{ width: 22, height: 22 }} />
          My Profile
        </div>

        <div className={`nav-item ${active === "create" && "active"}`}
          onClick={() => setActive("create")}>
          <img className = "me-2" src='/plus.png' style={{ width: 22, height: 22 }} />
          Create Capsule
        </div>

        <div className={`nav-item ${active === "vaults" && "active"}`}
          onClick={() => setActive("vaults")}>
          <img className = "me-2" src='/active-vault.png' style={{ width: 22, height: 22 }} />
          Active Vaults
        </div>

        <div className={`nav-item ${active === "legacy" && "active"}`}
          onClick={() => setActive("legacy")}>
          <img className = "me-2" src='/unlocked.png' style={{ width: 22, height: 22 }} />
          Legacy
        </div>

        <div className={`nav-item ${active === "history" && "active"}`}
          onClick={() => setActive("history")}>
          <img className = "me-2" src='/history.png' />
          History
        </div>

        <div className={`nav-item ${active === "upgrade" && "active"}`}
          onClick={() => setActive("upgrade")}>
          <img className = "me-2" src='/upgrade.png' />
          Upgrade Plan
        </div>
      </div>
