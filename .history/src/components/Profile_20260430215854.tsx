...
export default function Profile() {
  ...
  const handleAvatarUpdate = async (url: string) => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        "http://localhost:5000/auth/update-avatar", // Path matching your controller
        { avatarUrl: url },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      showAlert("SYSTEM IDENTITY UPDATED", "success");
    } catch (err) {
      showAlert("IDENTITY UPDATE FAILED", "error");
    }
  };
      ...
      <div className="identity-anchor-card">
        <div className="profile-main-info">
          <div className="avatar-wrapper" onClick={() => setIsPickerOpen(true)} style={{ cursor: 'pointer' }}>
            ...
            <img
              src={selectedAvatar || "/icons/user-avatar.png"}
              alt="Profile"
                ...
                <div className="avatar-grid">
                  {AVATAR_OPTIONS.map((url, index) => (
                    <div
                      ...
                      onClick={() => handleAvatarUpdate(url)} // Backend update function
                    >
