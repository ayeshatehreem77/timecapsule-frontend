...
export default function LoginModal() {
    ...
    const handleLogin = async () => {
        try {
            ...
            // 🔥 Backend se role uthayein (Agar backend bhej raha hai)
            // Agar backend sirf token bhejta hai, toh aapko response structure check karna hoga
            const user = res.data.user;
            const role = user?.role || "user";

            localStorage.setItem("token", token);
            localStorage.setItem("user", JSON.stringify(user)); // Full user save karein
