...
export default function Legacy() {
    ...
    const [userPasscode, setUserPasscode] = useState(""); // 1. Added passcode state
    ...
    const handleOpen = async (id: string) => {
        const passcode = prompt("Enter the secret passcode to decrypt:"); // 3. Quick way to get passcode
        if (!passcode) return;
        ...
        try {
            ...
            alert(`Message Decrypted: ${res.data.message}`);
            fetchLegacyCapsules(); // Refresh list to move this to History
        } catch (err) {
